...
Possibilities for <pol> are: UU, FF, LL, LT, TT, TL.
...
(of course using CMSSW_12_4_8).

Before compiling, check out this file from github:
...
- run "source cmsDriver.sh <number of events to generate>": this will generate a file called zzpol_cfg.py. Edit this with the correct gridpack location and
rename it to zzpol<pol>_cfg.py

- Edit runEvents.sh with the correct paths and run it as "bash runEvents.sh <pol>" (either interactively or on condor queues). This will give a file named out<pol>.yoda

- Do "./yodaToPlain.pl out<pol>.yoda" to change yoda output to the plain output desired by the COMETA authors.